...
| Stateless reset | Half of it is done, and the halves are worth separating. **Receiving one works**: a reset is indistinguishable from a short-header packet until its last 16 bytes are compared against a token the peer gave us, so `quic.connection` checks that *before* parsing rather than after failing to (§10.3), then enters draining and sends nothing further — §10.3.1, because a close aimed at a peer with no state only provokes another reset. Spare connection IDs are issued with tokens of their own, derived by HMAC from the server's seed so that possession cannot be guessed from anything observable. **Sending one is absent**, and the reason is not a branch: §10.3 is only useful if the token an endpoint would send matches one it issued *before* it lost its state, which means a key that survives a restart — an operational input this layer does not take. Deriving it from a per-process seed would produce resets no peer recognises, which is worse than sending nothing. |
...
carrying 281 of the repository's 759 tests plus seven of its 22 fuzz targets. All of
