...
| HTTP Datagrams (RFC 9297) | **Not done, and the missing piece is one layer down.** §2.1 conveys HTTP Datagrams in QUIC DATAGRAM frames, which are RFC 9221 — an extension this transport does not implement: there is no `max_datagram_frame_size` transport parameter and no 0x30/0x31 frame types, so there is nothing for §2.1's Quarter Stream ID to travel in. The Capsule Protocol (§3) would give a reliable substitute over the request stream and is the part that would also serve HTTP/2 and HTTP/1.1, but it is a stream encoding of the same absent feature rather than a way around it. Stating the order plainly because the dependency is easy to miss when reading RFC 9297 alone: RFC 9221 first, then §2.1's framing and the SETTINGS_H3_DATAGRAM (0x33) negotiation, then §3's capsules. |
