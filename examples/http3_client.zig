...
const Collector = struct {
    ...
    fn on(self: *Collector, conn: *http3.client.Connection, event: http3.client.Event) void {
        switch (event) {
            ...
            .goaway => {},
            // This example sends exactly one request, and it sends it on `established`
            // when the peer's initial allowance is untouched — so it can never be waiting
            // for credit and has nothing to do when more arrives.
            .stream_credit => {},
