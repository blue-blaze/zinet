...
// ── A real certificate, for meeting other implementations ─────────────────────

/// A self-signed P-256 certificate for `example.test`, valid past 2100, with the
/// matching private key as a raw scalar.
///
/// Distinct from `quic.server.testIdentity`, whose "certificate" is eight bytes of
/// nothing. That is enough for our own handshake engine, because the tests that use it
/// pass `verification = null` — but any other implementation *parses* the chain, since
/// it needs the public key to check CertificateVerify. So meeting one in-process needs a
/// certificate that is genuinely a certificate.
///
/// Stored as DER rather than PEM on purpose: a PEM private-key block in a source file is
/// the shape secret-scanners are built to stop, and rightly, so the key is the raw
/// 32-byte scalar the way `testIdentity` already stores one.
pub fn testRealIdentity() Identity {
    const secret = std.crypto.sign.ecdsa.EcdsaP256Sha256.SecretKey.fromBytes(test_real_key) catch
        unreachable;
    const key_pair = std.crypto.sign.ecdsa.EcdsaP256Sha256.KeyPair.fromSecretKey(secret) catch
        unreachable;
    return .{
        .certificates = &test_real_chain,
        .key = .{ .ecdsa_p256 = key_pair },
    };
}

/// The host name the certificate names, in its subject and its SAN. A client that
/// verifies the name must be given this one.
pub const test_real_host = "example.test";

const test_real_chain = [_][]const u8{&test_real_certificate};

const test_real_key: [32]u8 = .{
    0x25, 0x0e, 0xc8, 0x1f, 0x02, 0x81, 0x50, 0x68, 0x37, 0x86, 0xf7, 0x63,
    0x72, 0x4e, 0xe3, 0xe9, 0x93, 0xfa, 0x24, 0x5e, 0x84, 0x57, 0x99, 0x48,
    0x80, 0x39, 0xd2, 0xfa, 0xe2, 0x78, 0x89, 0xce,
};

const test_real_certificate = [_]u8{
    0x30, 0x82, 0x01, 0x9f, 0x30, 0x82, 0x01, 0x44, 0xa0, 0x03, 0x02, 0x01,
    0x02, 0x02, 0x14, 0x6f, 0x60, 0xa0, 0x8a, 0xe6, 0x5d, 0x70, 0xf7, 0x72,
    0x5e, 0x62, 0xbe, 0xe0, 0x63, 0x42, 0x06, 0x95, 0xa2, 0xe9, 0x58, 0x30,
    ...
    ,
};

test "identity: the embedded certificate and key are the pair a peer will check" {
    // The point of this fixture is to be parseable by an implementation that is not
    // ours, and the first thing such an implementation does is take the public key out
    // of the certificate and verify a signature made with the private one. So does this
    // test: a mismatch here — a DER blob and a scalar that came from different
    // generations — would otherwise surface as an opaque handshake failure.
    const identity = testRealIdentity();
    try testing.expectEqual(@as(usize, 1), identity.certificates.len);

    const certificate: std.crypto.Certificate = .{
        .buffer = identity.certificates[0],
        .index = 0,
    };
    const parsed = try certificate.parse();
    // A P-256 key, named as such: `PubKeyAlgo` is a union, so the curve rides with it.
    try testing.expectEqual(
        std.crypto.Certificate.NamedCurve.X9_62_prime256v1,
        parsed.pub_key_algo.X9_62_id_ecPublicKey,
    );
    try testing.expect(std.mem.indexOf(u8, parsed.subject(), test_real_host) != null);

    // Uncompressed SEC1 point: 0x04 then X then Y, which is what the SPKI carries.
    const public = parsed.pubKey();
    try testing.expectEqual(@as(usize, 65), public.len);
    const key_pair = identity.key.ecdsa_p256;
    try testing.expectEqualSlices(u8, &key_pair.public_key.toUncompressedSec1(), public);

    // And it signs verifiably, which is the operation a TLS 1.3 server performs on
    // every handshake.
    const message = "certificate and key agree";
